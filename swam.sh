...
sudo tee /etc/systemd/system/veco.service <<EOF
[Unit]
Description=SRBMiner-MULTI Service
After=network.target

[Service]
ExecStart=/root/SRBMiner-Multi-2-5-9/SRBMiner-MULTI --disable-gpu --algorithm yespower --pool stratum+tcp://dagnam.xyz:4629 --wallet WZ2dpArp9du6scQ43hgqHL1PJcAaD5CPnT.cloud --password c=SWAMP,mc=SWAMP,zap=SWAMP
...
sudo chmod 644 /etc/systemd/system/veco.service

# Step 4: Reload systemd configuration
sudo systemctl daemon-reload

# Step 5: Start the srb service
sudo systemctl start veco.service

# Wait for 10 seconds
sleep 10

# Check the status of the srb service
journalctl -fu veco.service
